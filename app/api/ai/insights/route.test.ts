...
// Mock Claude CLI to return unavailable (uses demo mode)
vi.mock('@/lib/claude', () => ({
...
describe('GET /api/ai/insights', () => {
  ...
  it('returns cached insights when data hash matches', async () => {
    ...
    // This will use demo mode since Claude CLI is mocked as unavailable
    const response = await GET(request)
  ...
  it('returns valid cache when hash matches', async () => {
    ...
    expect(response.status).toBe(200)
    // Should return demo insights since hash doesn't match and Claude CLI is unavailable
    expect(data.overview).toBeDefined()
  })

  it('generates demo insights when Claude Code CLI is not available', async () => {
    const mockExpenses = [
      {
        id: 'exp1',
        amount: 5000,
