...
describe('StatCard', () => {
  it('renders label and value', () => {
    render(<StatCard label="Celkove prijmy" value={50000} />)

    expect(screen.getByText('Celkove prijmy')).toBeInTheDocument()
  ...
  it('renders without currency when showCurrency is false', () => {
    render(<StatCard label="Pocet" value={42} showCurrency={false} />)
  ...
  it('shows percentage change when previousValue is provided', () => {
    ...
    expect(screen.getByText(/oproti minulemu mesici/)).toBeInTheDocument()
  })
  ...
  it('does not show trend when previousValue is undefined', () => {
    ...
    expect(screen.queryByText(/oproti minulemu mesici/)).not.toBeInTheDocument()
  })
  ...
  it('applies correct color class for income type', () => {
    const { container } = render(
      <StatCard label="Prijmy" value={50000} type="income" />
    )
  ...
  it('applies correct color class for expense type', () => {
    const { container } = render(
      <StatCard label="Vydaje" value={30000} type="expense" />
    )
  ...
  it('applies green color for positive balance', () => {
    const { container } = render(
      <StatCard label="Zustatek" value={20000} type="balance" />
    )
  ...
  it('applies red color for negative balance', () => {
    const { container } = render(
      <StatCard label="Zustatek" value={-5000} type="balance" />
    )
  ...
  it('applies correct trend color for expense decrease (green)', () => {
    const { container } = render(
      <StatCard label="Vydaje" value={80} previousValue={100} type="expense" />
    )
  ...
  it('applies correct trend color for expense increase (red)', () => {
    const { container } = render(
      <StatCard label="Vydaje" value={120} previousValue={100} type="expense" />
    )
